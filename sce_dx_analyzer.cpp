...
int main(int argc, char** argv)
{
  ...
  while(reader_tracks.Next())
      ...
      for(int k = 0; k < track_hit_ts.GetSize(); k++)
	{
      ...
      float slope = (max_time_x - min_time_x)/(max_time_z - min_time_z);

      // Calculate offset for each hit
      for(int i = 0; i < num_hits; i++)
	{
	  float ideal_x = slope*(selected_hit_zs[i] - min_time_z) + min_time_x;
	  //float ideal_z = (selected_hit_xs[i] - min_time_x)/slope + min_time_z;
	  float dx = selected_hit_xs[i] - ideal_x;

	  if(fabs(dx) < max_offset)
	    {
	    
	      if((selected_hit_ts[i] < min_time) || (selected_hit_ts[i] > max_time))
		{
		  continue;
		}
	      true_x = ideal_x;
